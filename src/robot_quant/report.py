...
def write_outputs(history: pd.DataFrame, state: dict, output_root: Path) -> None:
    ...
    _write_chart(history, reports_dir / "performance.png")
    (reports_dir / "latest.md").write_text(_markdown(state), encoding="utf-8")


def _write_chart(history: pd.DataFrame, path: Path) -> None:
...
- 从ETF可交易历史首日开始，初始投入¥{state['initial_contribution']:,.0f}。
- 预测策略首日没有前一交易日信号，因此先持有现金，不强制满仓。
- 每月首个交易日追加¥{state['monthly_contribution']:,.0f}。
- 收盘后产生信号，下一交易日开盘执行。
