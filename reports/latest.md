...
- 逻辑回归原始分数：**57.50%**
- 向50%收缩25%后，未来10个交易日跑赢沪深300的预测概率型分数：**51.88%**
- 模型原始研究仓位：**50%**（不可执行）
...
- 逐日成熟样本门控：**通过**；最近成熟样本50条，Brier=0.2471，AUC=0.5604；最近成熟窗口通过，仅允许不可执行影子卫星仓
- 不可执行影子目标仓位：**20%**。

## 基线优先门控

- 候选模型：**回顾性挑战者** `trend_risk_logistic_v3`；它是在查看既有历史后提出的候选，只能用未来新增数据继续验证。
- 模型研究概率：**51.88%**；批准概率固定为**50.00%**，表示当前没有可执行优势。
...
|---|---:|---:|---:|---|
| 5日收益 | 2.41% | -12.43% | 11.77% | 范围内 |
| 20日收益 | 11.41% | -24.36% | 25.38% | 范围内 |
| 60日收益 | -17.04% | -29.40% | 41.48% | 范围内 |
| 距20日均线 | 6.22% | -14.68% | 12.91% | 范围内 |
| 距60日均线 | -4.24% | -20.75% | 21.68% | 范围内 |
| 20日年化波动 | 40.85% | 11.93% | 82.45% | 范围内 |
| 成交量/20日均量 | 0.90倍 | 0.53倍 | 1.76倍 | 范围内 |
| 20日相对强弱 | 8.31% | -12.13% | 15.94% | 范围内 |
| 沪深300距120日均线 | 0.04% | -20.51% | 17.68% | 范围内 |
...
| 10日 | 38 | 未通过历史验证 | 预测未同时击败零收益MAE基线并达到高于50%的方向准确率，收益预测已停用 |
| 20日 | 27 | 未通过历史验证 | 预测未同时击败零收益MAE基线并达到高于50%的方向准确率，收益预测已停用 |
...
- 已实现的收缩后研究分数样本：**2429**。
- 全部重叠日标签的描述性方向准确率：**51.01%**。
- 每隔10个交易日抽取一次的近似非重叠样本：**243**；方向准确率：**51.85%**；Wilson 95%区间：**45.59% ～ 58.06%**。
- 收缩后概率型分数Brier：**0.2495**；原始模型：**0.2515**；固定50%概率基线：**0.2500**。Brier越低越好。
- Brier技能分数：**0.22%**（高于0才表示击败50%基线）；分段稳定性通过：**否**。
- ROC AUC：**0.5135**；0.5约等于随机排序。
...
| 最新尾窗 | 2026-05-25 | 2026-08-03 | 50 | 0.2464 | 0.5649 | 通过 |
...
|---|---:|---:|---:|---:|---:|
| 质量门控定投 | ¥16,000.00 | ¥17,509.31 | ¥1,509.31 | 9.43% | -7.36% |
| 固定定投 | ¥16,000.00 | ¥17,509.31 | ¥1,509.31 | 9.43% | -7.36% |
| 质量门控影子策略（不可执行） | ¥16,000.00 | ¥15,960.14 | ¥-39.86 | -0.25% | -1.33% |

质量门控定投相对固定定投的市值差：**¥0.00**

影子Alpha（影子策略相对固定定投的市值差）：**¥-1,549.17**。样本仅**22**个交易日，门控通过8日、实际持仓7日，当前只用于检验规则是否可能创造避险或超额收益。

![累计市值对比](performance.png)

## 数据口径与来源

- 模型特征与收益验证：`cnindex_official_980022`，覆盖2015-01-05至2026-08-17，共2824条。
