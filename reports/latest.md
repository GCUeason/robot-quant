...
- 逻辑回归原始分数：**59.36%**
- 向50%收缩25%后，未来10个交易日跑赢沪深300的预测概率型分数：**52.34%**
- 模型原始研究仓位：**50%**（不可执行）
...
- 逐日成熟样本门控：**未通过**；最近成熟样本50条，Brier=0.2519，AUC=0.4675；最近成熟窗口未同时通过Brier<0.25与AUC>0.5
- 不可执行影子目标仓位：**0%**。

## 基线优先门控

- 候选模型：**回顾性挑战者** `trend_risk_logistic_v3`；它是在查看既有历史后提出的候选，只能用未来新增数据继续验证。
- 模型研究概率：**52.34%**；批准概率固定为**50.00%**，表示当前没有可执行优势。
...
|---|---:|---:|---:|---|
| 5日收益 | 4.28% | -12.43% | 11.77% | 范围内 |
| 20日收益 | -4.16% | -24.36% | 25.38% | 范围内 |
| 60日收益 | -15.13% | -29.40% | 41.48% | 范围内 |
| 距20日均线 | 4.10% | -14.68% | 12.91% | 范围内 |
| 距60日均线 | -7.97% | -20.75% | 21.68% | 范围内 |
| 20日年化波动 | 49.07% | 11.93% | 82.45% | 范围内 |
| 成交量/20日均量 | 0.78倍 | 0.53倍 | 1.76倍 | 范围内 |
| 20日相对强弱 | -4.30% | -12.13% | 15.94% | 范围内 |
| 沪深300距120日均线 | -0.81% | -20.51% | 17.68% | 范围内 |
...
|---|---:|---|---:|---:|---:|
| 5日 | 40 | 样本达标 | 1.12% | -4.15% ～ 5.87% | 37.50% |
| 10日 | 40 | 样本达标 | -0.02% | -8.02% ～ 8.40% | 50.00% |
| 20日 | 40 | 样本达标 | -2.57% | -7.76% ～ 12.38% | 57.50% |

| 持有期 | 历史期间跌破起点概率 | 历史期间跌超5%概率 | 跌破后的低点窗口 |
|---|---:|---:|---:|
| 5日 | 55.00% | 10.00% | 第3～5日（n=22） |
| 10日 | 80.00% | 30.00% | 第4～9日（n=32） |
| 20日 | 85.00% | 47.50% | 第10～19日（n=34） |
...
验证状态：**极端动作样本不足**。规则在**151**个近似独立样本外日期上可计算；每次只使用该日以前已经实现的相似结果，相邻记录至少间隔10个交易日。
...
全部151个可验证日期的10日平均收益为0.72%、亏损率为49.01%、路径平均最差收益为-2.96%。

| 当时动作 | 独立触发次数 | 随后10日实际中位收益 | 随后10日实际平均收益 | 随后10日实际亏损率 | 路径平均最差收益 | 平均收益较全部日期 | 样本达标 | 方向一致 |
|---|---:|---:|---:|---:|---:|---:|---|---|
| 保持 | 80 | 0.04% | 0.86% | 48.75% | -2.86% | +0.14% | 是 | 是 |
| 观察 | 66 | 0.09% | 0.37% | 50.00% | -3.11% | -0.35% | 是 | 是 |
| 减仓 | 5 | 6.22% | 3.01% | 40.00% | -2.79% | +2.30% | 否 | 否 |
...
| 最新尾窗 | 2026-05-18 | 2026-07-27 | 50 | 0.2506 | 0.4992 | 失败 |
...
| 10日 | 151 | 4.96% | 4.84% | 49.67% | 65.56% | 未击败零收益基线 |
...
|---|---:|---:|---:|---:|---:|
| 质量门控定投 | ¥16,000.00 | ¥17,033.31 | ¥1,033.31 | 6.46% | -7.36% |
| 固定定投 | ¥16,000.00 | ¥17,033.31 | ¥1,033.31 | 6.46% | -7.36% |
| 质量门控影子策略（不可执行） | ¥16,000.00 | ¥15,856.70 | ¥-143.30 | -0.90% | -0.96% |

质量门控定投相对固定定投的市值差：**¥0.00**

影子Alpha（影子策略相对固定定投的市值差）：**¥-1,176.61**。样本仅**17**个交易日，门控通过3日、实际持仓3日，当前只用于检验规则是否可能创造避险或超额收益。

![累计市值对比](performance.png)

## 数据口径与来源

- 模型特征与收益验证：`cnindex_official_980022`，覆盖2015-01-05至2026-08-10，共2819条。
