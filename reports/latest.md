...
- 逻辑回归原始分数：**55.99%**
- 向50%收缩25%后，未来10个交易日跑赢沪深300的预测概率型分数：**51.50%**
- 模型原始研究仓位：**50%**（不可执行）
...
- 逐日成熟样本门控：**通过**；最近成熟样本50条，Brier=0.2498，AUC=0.6135；最近成熟窗口通过，仅允许不可执行影子卫星仓
- 不可执行影子目标仓位：**20%**。

## 基线优先门控

- 候选模型：**回顾性挑战者** `trend_risk_logistic_v3`；它是在查看既有历史后提出的候选，只能用未来新增数据继续验证。
- 模型研究概率：**51.50%**；批准概率固定为**50.00%**，表示当前没有可执行优势。
...
|---|---:|---:|---:|---|
| 5日收益 | 1.02% | -12.43% | 11.77% | 范围内 |
| 20日收益 | 7.69% | -24.36% | 25.38% | 范围内 |
| 60日收益 | -13.38% | -29.40% | 41.48% | 范围内 |
| 距20日均线 | -2.71% | -14.68% | 12.91% | 范围内 |
| 距60日均线 | -8.65% | -20.75% | 21.68% | 范围内 |
| 20日年化波动 | 47.90% | 11.93% | 82.45% | 范围内 |
| 成交量/20日均量 | 0.72倍 | 0.53倍 | 1.76倍 | 范围内 |
| 20日相对强弱 | 5.92% | -12.13% | 15.94% | 范围内 |
| 沪深300距120日均线 | -2.19% | -20.51% | 17.68% | 范围内 |
...
| 10日 | 39 | 未通过历史验证 | 预测未同时击败零收益MAE基线并达到高于50%的方向准确率，收益预测已停用 |
| 20日 | 26 | 未通过历史验证 | 预测未同时击败零收益MAE基线并达到高于50%的方向准确率，收益预测已停用 |
...
|---|---:|---|---:|---:|---:|
| 5日 | 40 | 样本达标 | 0.80% | -4.82% ～ 6.58% | 45.00% |
...
| 最新尾窗 | 2026-06-04 | 2026-08-13 | 50 | 0.2504 | 0.6282 | 失败 |
...
|---|---:|---:|---:|---:|---:|
| 质量门控定投 | ¥16,000.00 | ¥15,902.81 | ¥-97.19 | -0.61% | -12.23% |
| 固定定投 | ¥16,000.00 | ¥15,902.81 | ¥-97.19 | -0.61% | -12.23% |
| 质量门控影子策略（不可执行） | ¥16,000.00 | ¥15,655.70 | ¥-344.30 | -2.15% | -2.82% |

质量门控定投相对固定定投的市值差：**¥0.00**

影子Alpha（影子策略相对固定定投的市值差）：**¥-247.11**。样本仅**29**个交易日，门控通过15日、实际持仓14日，当前只用于检验规则是否可能创造避险或超额收益。

![累计市值对比](performance.png)

## 数据口径与来源

- 模型特征与收益验证：`cnindex_official_980022`，覆盖2015-01-05至2026-08-27，共2832条。
