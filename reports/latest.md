...
- 逻辑回归原始分数：**51.62%**
- 向50%收缩25%后，未来10个交易日跑赢沪深300的预测概率型分数：**50.41%**
- 模型原始研究仓位：**50%**（不可执行）
...
- 门控原因：回顾性挑战者尚未用新增数据证明长期优于固定定投；逐日成熟样本门控未通过：最近成熟窗口未同时通过Brier<0.25与AUC>0.5；当前样本外验证状态：unstable
- 逐日成熟样本门控：**未通过**；最近成熟样本50条，Brier=0.2548，AUC=0.7576；最近成熟窗口未同时通过Brier<0.25与AUC>0.5
- 不可执行影子目标仓位：**0%**。

## 基线优先门控

- 候选模型：**回顾性挑战者** `trend_risk_logistic_v3`；它是在查看既有历史后提出的候选，只能用未来新增数据继续验证。
- 模型研究概率：**50.41%**；批准概率固定为**50.00%**，表示当前没有可执行优势。
- 本月训练集内部隔离验证：样本2250条，Brier=0.2492，AUC=0.5188，**通过**。
...
以下边界来自本月模型实际使用的 **2705** 条训练样本；不是另算的一套口径。

| 特征 | 当前值 | 训练P1 | 训练P99 | 判定 |
|---|---:|---:|---:|---|
| 5日收益 | 0.06% | -12.41% | 11.89% | 范围内 |
| 20日收益 | -2.81% | -24.38% | 25.37% | 范围内 |
| 60日收益 | -17.87% | -29.34% | 41.42% | 范围内 |
| 距20日均线 | -1.43% | -15.05% | 12.90% | 范围内 |
| 距60日均线 | -7.36% | -20.62% | 21.56% | 范围内 |
| 20日年化波动 | 25.01% | 11.95% | 82.39% | 范围内 |
| 成交量/20日均量 | 0.95倍 | 0.53倍 | 1.76倍 | 范围内 |
| 20日相对强弱 | 0.08% | -12.50% | 15.92% | 范围内 |
| 沪深300距120日均线 | -5.63% | -20.33% | 17.67% | 范围内 |
...
| 20日 | 35 | 未通过历史验证 | 预测未同时击败零收益MAE基线并达到高于50%的方向准确率，收益预测已停用 |
...
|---|---:|---|---:|---:|---:|
| 5日 | 40 | 样本达标 | -0.10% | -4.96% ～ 6.58% | 50.00% |
| 10日 | 40 | 样本达标 | -0.20% | -5.96% ～ 9.97% | 50.00% |
| 20日 | 40 | 样本达标 | -0.34% | -11.28% ～ 9.39% | 50.00% |

| 持有期 | 历史期间跌破起点概率 | 历史期间跌超5%概率 | 跌破后的低点窗口 |
|---|---:|---:|---:|
| 5日 | 80.00% | 17.50% | 第1～4日（n=32） |
| 10日 | 80.00% | 30.00% | 第2～9日（n=32） |
| 20日 | 87.50% | 55.00% | 第8～18日（n=35） |
...
验证状态：**极端动作样本不足**。规则在**153**个近似独立样本外日期上可计算；每次只使用该日以前已经实现的相似结果，相邻记录至少间隔10个交易日。
...
全部153个可验证日期的10日平均收益为0.67%、亏损率为49.02%、路径平均最差收益为-2.97%。

| 当时动作 | 独立触发次数 | 随后10日实际中位收益 | 随后10日实际平均收益 | 随后10日实际亏损率 | 路径平均最差收益 | 平均收益较全部日期 | 样本达标 | 方向一致 |
|---|---:|---:|---:|---:|---:|---:|---|---|
| 保持 | 80 | 0.04% | 0.86% | 48.75% | -2.86% | +0.19% | 是 | 是 |
| 观察 | 68 | 0.09% | 0.28% | 50.00% | -3.12% | -0.39% | 是 | 是 |
| 减仓 | 5 | 6.22% | 3.01% | 40.00% | -2.79% | +2.34% | 否 | 否 |
...
- 已实现的收缩后研究分数样本：**2452**。
- 全部重叠日标签的描述性方向准确率：**50.65%**。
- 每隔10个交易日抽取一次的近似非重叠样本：**246**；方向准确率：**51.22%**；Wilson 95%区间：**45.00% ～ 57.40%**。
- 收缩后概率型分数Brier：**0.2496**；原始模型：**0.2522**；固定50%概率基线：**0.2500**。Brier越低越好。
- Brier技能分数：**0.15%**（高于0才表示击败50%基线）；分段稳定性通过：**否**。
- ROC AUC：**0.5081**；0.5约等于随机排序。

### 模型分段稳定性

每50个连续已实现预测形成一个固定分段，至少需要3个固定分段。当前共49个固定分段，通过24个；下表仅展示最近窗口，完整结果保留在 `data/latest_state.json`。最新尾段不足40条时，额外检查最近50条滚动尾窗，但该重叠尾窗不计入独立分段数量。
...
| 固定分段 | 2026-06-24 | 2026-09-01 | 50 | 0.2543 | 0.7135 | 失败 |
| 最新尾窗 | 2026-06-26 | 2026-09-03 | 50 | 0.2555 | 0.7831 | 失败 |
...
| 上涨/高波动 | 261 | 0.2469 | 0.6056 | 通过 |
| 下跌/低波动 | 888 | 0.2504 | 0.4724 | 失败 |
| 下跌/高波动 | 177 | 0.2465 | 0.5459 | 通过 |
...
|---|---:|---:|---:|---:|---:|---|
| 5日 | 338 | 3.43% | 3.31% | 48.22% | 70.71% | 未击败零收益基线 |
| 10日 | 153 | 4.94% | 4.84% | 50.33% | 66.01% | 未击败零收益基线 |
| 20日 | 60 | 7.48% | 7.27% | 58.33% | 70.00% | 未击败零收益基线 |
...
|---|---:|---:|---:|---:|---:|
| 质量门控定投 | ¥17,000.00 | ¥16,275.04 | ¥-724.96 | -4.26% | -14.55% |
| 固定定投 | ¥17,000.00 | ¥16,275.04 | ¥-724.96 | -4.26% | -14.55% |
| 质量门控影子策略（不可执行） | ¥17,000.00 | ¥16,582.50 | ¥-417.50 | -2.46% | -2.82% |

质量门控定投相对固定定投的市值差：**¥0.00**

影子Alpha（影子策略相对固定定投的市值差）：**¥307.46**。样本仅**44**个交易日，门控通过15日、实际持仓15日，当前只用于检验规则是否可能创造避险或超额收益。

![累计市值对比](performance.png)

## 数据口径与来源

- 模型特征与收益验证：`cnindex_official_980022`，覆盖2015-01-05至2026-09-17，共2847条。
