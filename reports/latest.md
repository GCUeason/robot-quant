...
- 逻辑回归原始分数：**61.57%**
- 向50%收缩25%后，未来10个交易日跑赢沪深300的预测概率型分数：**52.89%**
- 模型原始研究仓位：**50%**（不可执行）
...
- 逐日成熟样本门控：**通过**；最近成熟样本50条，Brier=0.2491，AUC=0.5296；最近成熟窗口通过，仅允许不可执行影子卫星仓
- 不可执行影子目标仓位：**20%**。

## 基线优先门控

- 候选模型：**回顾性挑战者** `trend_risk_logistic_v3`；它是在查看既有历史后提出的候选，只能用未来新增数据继续验证。
- 模型研究概率：**52.89%**；批准概率固定为**50.00%**，表示当前没有可执行优势。
...
|---|---:|---:|---:|---|
| 5日收益 | -0.30% | -12.43% | 11.77% | 范围内 |
| 20日收益 | -2.85% | -24.36% | 25.38% | 范围内 |
| 60日收益 | -17.09% | -29.40% | 41.48% | 范围内 |
| 距20日均线 | 4.18% | -14.68% | 12.91% | 范围内 |
| 距60日均线 | -7.42% | -20.75% | 21.68% | 范围内 |
| 20日年化波动 | 49.90% | 11.93% | 82.45% | 范围内 |
| 成交量/20日均量 | 0.92倍 | 0.53倍 | 1.76倍 | 范围内 |
| 20日相对强弱 | -2.12% | -12.13% | 15.94% | 范围内 |
| 沪深300距120日均线 | -1.58% | -20.51% | 17.68% | 范围内 |
...
|---|---:|---|---:|---:|---:|
| 5日 | 40 | 样本达标 | 0.30% | -4.79% ～ 6.55% | 45.00% |
| 10日 | 40 | 样本达标 | -0.02% | -8.02% ～ 9.61% | 50.00% |
| 20日 | 40 | 样本达标 | -2.30% | -8.46% ～ 12.38% | 57.50% |

| 持有期 | 历史期间跌破起点概率 | 历史期间跌超5%概率 | 跌破后的低点窗口 |
|---|---:|---:|---:|
| 5日 | 57.50% | 12.50% | 第2～5日（n=23） |
| 10日 | 77.50% | 30.00% | 第4～10日（n=31） |
| 20日 | 85.00% | 50.00% | 第10～19日（n=34） |
...
- 已实现的收缩后研究分数样本：**2427**。
- 全部重叠日标签的描述性方向准确率：**50.97%**。
- 每隔10个交易日抽取一次的近似非重叠样本：**243**；方向准确率：**51.85%**；Wilson 95%区间：**45.59% ～ 58.06%**。
- 收缩后概率型分数Brier：**0.2495**；原始模型：**0.2517**；固定50%概率基线：**0.2500**。Brier越低越好。
- Brier技能分数：**0.21%**（高于0才表示击败50%基线）；分段稳定性通过：**否**。
- ROC AUC：**0.5127**；0.5约等于随机排序。
...
| 最新尾窗 | 2026-05-21 | 2026-07-30 | 50 | 0.2483 | 0.5369 | 通过 |
...
|---|---:|---:|---:|---:|---:|---|
| 5日 | 333 | 3.43% | 3.30% | 47.75% | 70.57% | 未击败零收益基线 |
...
|---|---:|---:|---:|---:|---:|
| 质量门控定投 | ¥16,000.00 | ¥16,640.61 | ¥640.61 | 4.00% | -7.36% |
| 固定定投 | ¥16,000.00 | ¥16,640.61 | ¥640.61 | 4.00% | -7.36% |
| 质量门控影子策略（不可执行） | ¥16,000.00 | ¥15,799.54 | ¥-200.46 | -1.25% | -1.33% |

质量门控定投相对固定定投的市值差：**¥0.00**

影子Alpha（影子策略相对固定定投的市值差）：**¥-841.07**。样本仅**20**个交易日，门控通过6日、实际持仓5日，当前只用于检验规则是否可能创造避险或超额收益。

![累计市值对比](performance.png)

## 数据口径与来源

- 模型特征与收益验证：`cnindex_official_980022`，覆盖2015-01-05至2026-08-13，共2822条。
