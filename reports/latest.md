...
- 逻辑回归原始分数：**60.97%**
- 向50%收缩25%后，未来10个交易日跑赢沪深300的预测概率型分数：**52.74%**
- 模型原始研究仓位：**50%**（不可执行）
...
- 逐日成熟样本门控：**通过**；最近成熟样本50条，Brier=0.2464，AUC=0.5649；最近成熟窗口通过，仅允许不可执行影子卫星仓
- 不可执行影子目标仓位：**20%**。

## 基线优先门控

- 候选模型：**回顾性挑战者** `trend_risk_logistic_v3`；它是在查看既有历史后提出的候选，只能用未来新增数据继续验证。
- 模型研究概率：**52.74%**；批准概率固定为**50.00%**，表示当前没有可执行优势。
...
|---|---:|---:|---:|---|
| 5日收益 | 2.79% | -12.43% | 11.77% | 范围内 |
| 20日收益 | 9.96% | -24.36% | 25.38% | 范围内 |
| 60日收益 | -15.17% | -29.40% | 41.48% | 范围内 |
| 距20日均线 | 8.31% | -14.68% | 12.91% | 范围内 |
| 距60日均线 | -1.59% | -20.75% | 21.68% | 范围内 |
| 20日年化波动 | 39.69% | 11.93% | 82.45% | 范围内 |
| 成交量/20日均量 | 1.24倍 | 0.53倍 | 1.76倍 | 范围内 |
| 20日相对强弱 | 10.25% | -12.13% | 15.94% | 范围内 |
| 沪深300距120日均线 | -0.29% | -20.51% | 17.68% | 范围内 |
...
| 持有期 | 严格非重叠样本 | 状态 | 原因 |
|---|---:|---|---|
| 5日 | 28 | 未通过历史验证 | 预测未同时击败零收益MAE基线并达到高于50%的方向准确率，收益预测已停用 |
| 10日 | 18 | 样本不足 | 去除10日重叠路径后有效样本不足20条（当前18条） |
| 20日 | 12 | 样本不足 | 去除20日重叠路径后有效样本不足20条（当前12条） |
...
|---|---:|---|---:|---:|---:|
| 5日 | 40 | 样本达标 | 0.80% | -4.82% ～ 6.69% | 45.00% |
| 10日 | 40 | 样本达标 | 0.62% | -5.96% ～ 9.97% | 50.00% |
| 20日 | 40 | 样本达标 | -0.34% | -11.28% ～ 9.39% | 50.00% |

| 持有期 | 历史期间跌破起点概率 | 历史期间跌超5%概率 | 跌破后的低点窗口 |
|---|---:|---:|---:|
| 5日 | 77.50% | 17.50% | 第1～4日（n=31） |
...
- 已实现的收缩后研究分数样本：**2430**。
- 全部重叠日标签的描述性方向准确率：**51.03%**。
- 每隔10个交易日抽取一次的近似非重叠样本：**243**；方向准确率：**51.85%**；Wilson 95%区间：**45.59% ～ 58.06%**。
- 收缩后概率型分数Brier：**0.2494**；原始模型：**0.2515**；固定50%概率基线：**0.2500**。Brier越低越好。
- Brier技能分数：**0.22%**（高于0才表示击败50%基线）；分段稳定性通过：**否**。
- ROC AUC：**0.5139**；0.5约等于随机排序。
...
| 最新尾窗 | 2026-05-26 | 2026-08-04 | 50 | 0.2455 | 0.5990 | 通过 |
...
|---|---:|---:|---:|---:|---:|---|
| 5日 | 334 | 3.42% | 3.30% | 47.60% | 70.66% | 未击败零收益基线 |
...
|---|---:|---:|---:|---:|---:|
| 质量门控定投 | ¥16,000.00 | ¥15,914.71 | ¥-85.29 | -0.53% | -9.11% |
| 固定定投 | ¥16,000.00 | ¥15,914.71 | ¥-85.29 | -0.53% | -9.11% |
| 质量门控影子策略（不可执行） | ¥16,000.00 | ¥15,671.27 | ¥-328.73 | -2.05% | -2.11% |

质量门控定投相对固定定投的市值差：**¥0.00**

影子Alpha（影子策略相对固定定投的市值差）：**¥-243.44**。样本仅**23**个交易日，门控通过9日、实际持仓8日，当前只用于检验规则是否可能创造避险或超额收益。

![累计市值对比](performance.png)

## 数据口径与来源

- 模型特征与收益验证：`cnindex_official_980022`，覆盖2015-01-05至2026-08-18，共2825条。
