...
- 逐日成熟样本门控：**通过**；最近成熟样本50条，Brier=0.2483，AUC=0.5369；最近成熟窗口通过，仅允许不可执行影子卫星仓
- 不可执行影子目标仓位：**20%**。
...
|---|---:|---:|---:|---|
| 5日收益 | -1.20% | -12.43% | 11.77% | 范围内 |
| 20日收益 | 4.62% | -24.36% | 25.38% | 范围内 |
| 60日收益 | -17.95% | -29.40% | 41.48% | 范围内 |
| 距20日均线 | 4.11% | -14.68% | 12.91% | 范围内 |
| 距60日均线 | -6.96% | -20.75% | 21.68% | 范围内 |
| 20日年化波动 | 42.75% | 11.93% | 82.45% | 范围内 |
| 成交量/20日均量 | 0.75倍 | 0.53倍 | 1.76倍 | 范围内 |
| 20日相对强弱 | 1.60% | -12.13% | 15.94% | 范围内 |
| 沪深300距120日均线 | -1.53% | -20.51% | 17.68% | 范围内 |
...
- 已实现的收缩后研究分数样本：**2428**。
- 全部重叠日标签的描述性方向准确率：**50.99%**。
- 每隔10个交易日抽取一次的近似非重叠样本：**243**；方向准确率：**51.85%**；Wilson 95%区间：**45.59% ～ 58.06%**。
- 收缩后概率型分数Brier：**0.2495**；原始模型：**0.2516**；固定50%概率基线：**0.2500**。Brier越低越好。
- Brier技能分数：**0.21%**（高于0才表示击败50%基线）；分段稳定性通过：**否**。
- ROC AUC：**0.5131**；0.5约等于随机排序。
...
| 最新尾窗 | 2026-05-22 | 2026-07-31 | 50 | 0.2471 | 0.5604 | 通过 |
...
|---|---:|---:|---:|---:|---:|
| 质量门控定投 | ¥16,000.00 | ¥17,080.91 | ¥1,080.91 | 6.76% | -7.36% |
| 固定定投 | ¥16,000.00 | ¥17,080.91 | ¥1,080.91 | 6.76% | -7.36% |
| 质量门控影子策略（不可执行） | ¥16,000.00 | ¥15,880.94 | ¥-119.06 | -0.74% | -1.33% |

质量门控定投相对固定定投的市值差：**¥0.00**

影子Alpha（影子策略相对固定定投的市值差）：**¥-1,199.97**。样本仅**21**个交易日，门控通过7日、实际持仓6日，当前只用于检验规则是否可能创造避险或超额收益。

![累计市值对比](performance.png)

## 数据口径与来源

- 模型特征与收益验证：`cnindex_official_980022`，覆盖2015-01-05至2026-08-14，共2823条。
