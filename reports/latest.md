...
- 逻辑回归原始分数：**64.64%**
- 向50%收缩25%后，未来10个交易日跑赢沪深300的预测概率型分数：**53.66%**
- 模型原始研究仓位：**50%**（不可执行）
- 当前使用模型：`shrunk_logistic_regression`
- 当前状态：**训练分布外，模型条件预测停用**（distance_ma20:below_1pct）；下方仅展示不可执行的历史压力参考

## 下一交易日执行计划

- 可执行目标仓位：**100%**
- 执行政策：**固定定投是唯一可执行模拟**；模型尚未获得仓位控制权。
- 门控原因：回顾性挑战者尚未用新增数据证明长期优于固定定投；逐日成熟样本门控未通过：当前训练分布外：distance_ma20:below_1pct；当前样本外验证状态：unstable；当前训练分布外特征：distance_ma20:below_1pct
- 逐日成熟样本门控：**未通过**；最近成熟样本50条，Brier=0.2485，AUC=0.5308；当前训练分布外：distance_ma20:below_1pct
- 不可执行影子目标仓位：**0%**。

## 基线优先门控

- 候选模型：**回顾性挑战者** `trend_risk_logistic_v3`；它是在查看既有历史后提出的候选，只能用未来新增数据继续验证。
- 模型研究概率：**53.66%**；批准概率固定为**50.00%**，表示当前没有可执行优势。
- 本月训练集内部隔离验证：样本2215条，Brier=0.2492，AUC=0.5162，**通过**。
...
以下边界来自本月模型实际使用的 **2661** 条训练样本；不是另算的一套口径。

| 特征 | 当前值 | 训练P1 | 训练P99 | 判定 |
|---|---:|---:|---:|---|
| 5日收益 | -6.56% | -12.31% | 11.68% | 范围内 |
| 20日收益 | -16.28% | -24.37% | 25.39% | 范围内 |
| 60日收益 | -15.73% | -29.47% | 41.55% | 范围内 |
| 距20日均线 | -15.93% | -14.71% | 12.91% | 低于训练P1 |
| 距60日均线 | -20.13% | -20.92% | 21.85% | 范围内 |
| 20日年化波动 | 66.01% | 11.90% | 82.45% | 范围内 |
| 成交量/20日均量 | 0.53倍 | 0.53倍 | 1.76倍 | 范围内 |
| 20日相对强弱 | -11.79% | -12.14% | 15.96% | 范围内 |
| 沪深300距120日均线 | -2.04% | -20.69% | 17.70% | 范围内 |
...
| 持有期 | 严格非重叠样本 | 状态 | 原因 |
|---|---:|---|---|
| 5日 | 0 | 暂无足够样本 | 当前状态处于训练分布外：distance_ma20:below_1pct |
| 10日 | 0 | 暂无足够样本 | 当前状态处于训练分布外：distance_ma20:below_1pct |
| 20日 | 0 | 暂无足够样本 | 当前状态处于训练分布外：distance_ma20:below_1pct |
...
|---|---:|---|---:|---:|---:|
| 5日 | 40 | 样本达标 | 0.70% | -4.72% ～ 6.47% | 40.00% |
| 10日 | 40 | 样本达标 | 0.77% | -8.02% ～ 8.40% | 47.50% |
| 20日 | 40 | 样本达标 | -1.43% | -8.46% ～ 12.38% | 55.00% |

| 持有期 | 历史期间跌破起点概率 | 历史期间跌超5%概率 | 跌破后的低点窗口 |
|---|---:|---:|---:|
| 5日 | 55.00% | 12.50% | 第3～5日（n=22） |
| 10日 | 80.00% | 32.50% | 第4～9日（n=32） |
| 20日 | 82.50% | 47.50% | 第10～19日（n=33） |

“什么时候会掉”不能精确到某一天；低点窗口只对曾跌破起始价格的历史路径统计，并给出低点日的25%～75%分位及条件样本数，不混入全程上涨路径。每天收盘后重算，优先看亏损概率和收益下界是否同步恶化。

## 研究型卖出指标（不可直接执行）

- 当前动作：**不可用**；原因：当前状态处于训练分布外：distance_ma20:below_1pct。
...
验证状态：**极端动作样本不足**。规则在**150**个近似独立样本外日期上可计算；每次只使用该日以前已经实现的相似结果，相邻记录至少间隔10个交易日。
...
全部150个可验证日期的10日平均收益为0.67%、亏损率为49.33%、路径平均最差收益为-2.96%。

| 当时动作 | 独立触发次数 | 随后10日实际中位收益 | 随后10日实际平均收益 | 随后10日实际亏损率 | 路径平均最差收益 | 平均收益较全部日期 | 样本达标 | 方向一致 |
|---|---:|---:|---:|---:|---:|---:|---|---|
| 保持 | 80 | 0.04% | 0.86% | 48.75% | -2.86% | +0.19% | 是 | 是 |
| 观察 | 65 | -0.01% | 0.25% | 50.77% | -3.09% | -0.42% | 是 | 是 |
| 减仓 | 5 | 6.22% | 3.01% | 40.00% | -2.79% | +2.35% | 否 | 否 |
...
- 已实现的收缩后研究分数样本：**2413**。
- 全部重叠日标签的描述性方向准确率：**50.89%**。
- 每隔10个交易日抽取一次的近似非重叠样本：**242**；方向准确率：**51.65%**；Wilson 95%区间：**45.38% ～ 57.87%**。
- 收缩后概率型分数Brier：**0.2496**；原始模型：**0.2519**；固定50%概率基线：**0.2500**。Brier越低越好。
- Brier技能分数：**0.18%**（高于0才表示击败50%基线）；分段稳定性通过：**否**。
- ROC AUC：**0.5108**；0.5约等于随机排序。
...
| 最新尾窗 | 2026-04-28 | 2026-07-10 | 50 | 0.2492 | 0.5121 | 通过 |
...
| 上涨/高波动 | 258 | 0.2466 | 0.6145 | 通过 |
| 下跌/低波动 | 888 | 0.2504 | 0.4724 | 失败 |
| 下跌/高波动 | 141 | 0.2452 | 0.5424 | 通过 |
...
|---|---:|---:|---:|---:|---:|---|
| 5日 | 331 | 3.42% | 3.30% | 47.73% | 70.39% | 未击败零收益基线 |
| 10日 | 150 | 4.93% | 4.82% | 50.00% | 66.00% | 未击败零收益基线 |
| 20日 | 58 | 7.24% | 6.98% | 58.62% | 70.69% | 未击败零收益基线 |
...
| 质量门控影子策略（不可执行） | ¥15,000.00 | ¥14,880.67 | ¥-119.33 | -0.80% | -0.80% |

质量门控定投相对固定定投的市值差：**¥0.00**

影子Alpha（影子策略相对固定定投的市值差）：**¥925.43**。样本仅**5**个交易日，门控通过2日、实际持仓2日，当前只用于检验规则是否可能创造避险或超额收益。

![累计市值对比](performance.png)

## 数据口径与来源

- 模型特征与收益验证：`cnindex_official_980022`，覆盖2015-01-05至2026-07-24，共2808条。
