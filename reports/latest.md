...
- 逻辑回归原始分数：**63.23%**
- 向50%收缩25%后，未来10个交易日跑赢沪深300的预测概率型分数：**53.31%**
- 模型原始研究仓位：**50%**（不可执行）
- 当前使用模型：`shrunk_logistic_regression`
- 当前状态：**训练分布外，模型条件预测停用**（return_5:above_99pct）；下方仅展示不可执行的历史压力参考

## 下一交易日执行计划

- 可执行目标仓位：**100%**
- 执行政策：**固定定投是唯一可执行模拟**；模型尚未获得仓位控制权。
- 门控原因：回顾性挑战者尚未用新增数据证明长期优于固定定投；逐日成熟样本门控未通过：当前训练分布外：return_5:above_99pct；当前样本外验证状态：unstable；当前训练分布外特征：return_5:above_99pct
- 逐日成熟样本门控：**未通过**；最近成熟样本50条，Brier=0.2527，AUC=0.4221；当前训练分布外：return_5:above_99pct
- 不可执行影子目标仓位：**0%**。

## 基线优先门控

- 候选模型：**回顾性挑战者** `trend_risk_logistic_v3`；它是在查看既有历史后提出的候选，只能用未来新增数据继续验证。
- 模型研究概率：**53.31%**；批准概率固定为**50.00%**，表示当前没有可执行优势。
...
|---|---:|---:|---:|---|
| 5日收益 | 12.91% | -12.43% | 11.77% | 高于训练P99 |
| 20日收益 | -9.64% | -24.36% | 25.38% | 范围内 |
| 60日收益 | -13.71% | -29.40% | 41.48% | 范围内 |
| 距20日均线 | 3.29% | -14.68% | 12.91% | 范围内 |
| 距60日均线 | -8.40% | -20.75% | 21.68% | 范围内 |
| 20日年化波动 | 53.47% | 11.93% | 82.45% | 范围内 |
| 成交量/20日均量 | 0.98倍 | 0.53倍 | 1.76倍 | 范围内 |
| 20日相对强弱 | -5.02% | -12.13% | 15.94% | 范围内 |
| 沪深300距120日均线 | -1.86% | -20.51% | 17.68% | 范围内 |
...
| 持有期 | 严格非重叠样本 | 状态 | 原因 |
|---|---:|---|---|
| 5日 | 0 | 暂无足够样本 | 当前状态处于训练分布外：return_5:above_99pct |
| 10日 | 0 | 暂无足够样本 | 当前状态处于训练分布外：return_5:above_99pct |
| 20日 | 0 | 暂无足够样本 | 当前状态处于训练分布外：return_5:above_99pct |
...
|---|---:|---|---:|---:|---:|
| 5日 | 40 | 样本达标 | 0.70% | -4.79% ～ 6.55% | 42.50% |
| 10日 | 40 | 样本达标 | 0.77% | -8.02% ～ 8.40% | 47.50% |
| 20日 | 40 | 样本达标 | -1.43% | -8.45% ～ 13.03% | 55.00% |

| 持有期 | 历史期间跌破起点概率 | 历史期间跌超5%概率 | 跌破后的低点窗口 |
|---|---:|---:|---:|
| 5日 | 55.00% | 12.50% | 第2～5日（n=22） |
| 10日 | 80.00% | 30.00% | 第4～9日（n=32） |
| 20日 | 85.00% | 47.50% | 第10～18日（n=34） |

“什么时候会掉”不能精确到某一天；低点窗口只对曾跌破起始价格的历史路径统计，并给出低点日的25%～75%分位及条件样本数，不混入全程上涨路径。每天收盘后重算，优先看亏损概率和收益下界是否同步恶化。

## 研究型卖出指标（不可直接执行）

- 当前动作：**不可用**；原因：当前状态处于训练分布外：return_5:above_99pct。
...
- 已实现的收缩后研究分数样本：**2422**。
- 全部重叠日标签的描述性方向准确率：**50.87%**。
- 每隔10个交易日抽取一次的近似非重叠样本：**243**；方向准确率：**51.85%**；Wilson 95%区间：**45.59% ～ 58.06%**。
- 收缩后概率型分数Brier：**0.2496**；原始模型：**0.2519**；固定50%概率基线：**0.2500**。Brier越低越好。
- Brier技能分数：**0.18%**（高于0才表示击败50%基线）；分段稳定性通过：**否**。
- ROC AUC：**0.5107**；0.5约等于随机排序。
...
| 最新尾窗 | 2026-05-14 | 2026-07-23 | 50 | 0.2523 | 0.4497 | 失败 |
...
|---|---:|---:|---:|---:|---:|---|
| 5日 | 332 | 3.42% | 3.30% | 47.89% | 70.48% | 未击败零收益基线 |
...
|---|---:|---:|---:|---:|---:|
| 质量门控定投 | ¥16,000.00 | ¥16,866.71 | ¥866.71 | 5.42% | -7.36% |
| 固定定投 | ¥16,000.00 | ¥16,866.71 | ¥866.71 | 5.42% | -7.36% |
| 质量门控影子策略（不可执行） | ¥16,000.00 | ¥15,856.70 | ¥-143.30 | -0.90% | -0.96% |

质量门控定投相对固定定投的市值差：**¥0.00**

影子Alpha（影子策略相对固定定投的市值差）：**¥-1,010.01**。样本仅**15**个交易日，门控通过3日、实际持仓3日，当前只用于检验规则是否可能创造避险或超额收益。

![累计市值对比](performance.png)

## 数据口径与来源

- 模型特征与收益验证：`cnindex_official_980022`，覆盖2015-01-05至2026-08-06，共2817条。
