...
- 逻辑回归原始分数：**49.04%**
- 向50%收缩25%后，未来10个交易日跑赢沪深300的预测概率型分数：**49.76%**
- 模型原始研究仓位：**0%**（不可执行）
...
- 逐日成熟样本门控：**未通过**；最近成熟样本50条，Brier=0.2555，AUC=0.7996；最近成熟窗口未同时通过Brier<0.25与AUC>0.5
- 不可执行影子目标仓位：**0%**。

## 基线优先门控

- 候选模型：**回顾性挑战者** `trend_risk_logistic_v3`；它是在查看既有历史后提出的候选，只能用未来新增数据继续验证。
- 模型研究概率：**49.76%**；批准概率固定为**50.00%**，表示当前没有可执行优势。
...
|---|---:|---:|---:|---|
| 5日收益 | 0.82% | -12.41% | 11.89% | 范围内 |
| 20日收益 | -3.00% | -24.38% | 25.37% | 范围内 |
| 60日收益 | -20.65% | -29.34% | 41.42% | 范围内 |
| 距20日均线 | -0.36% | -15.05% | 12.90% | 范围内 |
| 距60日均线 | -5.06% | -20.62% | 21.56% | 范围内 |
| 20日年化波动 | 20.47% | 11.95% | 82.39% | 范围内 |
| 成交量/20日均量 | 0.90倍 | 0.53倍 | 1.76倍 | 范围内 |
| 20日相对强弱 | 1.13% | -12.50% | 15.92% | 范围内 |
| 沪深300距120日均线 | -6.09% | -20.33% | 17.67% | 范围内 |
...
| 最新尾窗 | 2026-07-03 | 2026-09-10 | 50 | 0.2548 | 0.7843 | 失败 |
...
|---|---:|---:|---:|---:|---:|
| 质量门控定投 | ¥17,000.00 | ¥16,375.84 | ¥-624.16 | -3.67% | -14.55% |
| 固定定投 | ¥17,000.00 | ¥16,375.84 | ¥-624.16 | -3.67% | -14.55% |
| 质量门控影子策略（不可执行） | ¥17,000.00 | ¥16,582.50 | ¥-417.50 | -2.46% | -2.82% |

质量门控定投相对固定定投的市值差：**¥0.00**

影子Alpha（影子策略相对固定定投的市值差）：**¥206.66**。样本仅**49**个交易日，门控通过15日、实际持仓15日，当前只用于检验规则是否可能创造避险或超额收益。

![累计市值对比](performance.png)

## 数据口径与来源

- 模型特征与收益验证：`cnindex_official_980022`，覆盖2015-01-05至2026-09-24，共2852条。
