...
- 逻辑回归原始分数：**51.20%**
- 向50%收缩25%后，未来10个交易日跑赢沪深300的预测概率型分数：**50.30%**
- 模型原始研究仓位：**50%**（不可执行）
...
- 逐日成熟样本门控：**未通过**；最近成熟样本50条，Brier=0.2560，AUC=0.8514；最近成熟窗口未同时通过Brier<0.25与AUC>0.5
- 不可执行影子目标仓位：**0%**。

## 基线优先门控

- 候选模型：**回顾性挑战者** `trend_risk_logistic_v3`；它是在查看既有历史后提出的候选，只能用未来新增数据继续验证。
- 模型研究概率：**50.30%**；批准概率固定为**50.00%**，表示当前没有可执行优势。
...
|---|---:|---:|---:|---|
| 5日收益 | 4.26% | -12.41% | 11.89% | 范围内 |
| 20日收益 | -0.90% | -24.38% | 25.37% | 范围内 |
| 60日收益 | -18.29% | -29.34% | 41.42% | 范围内 |
| 距20日均线 | 0.52% | -15.05% | 12.90% | 范围内 |
| 距60日均线 | -4.83% | -20.62% | 21.56% | 范围内 |
| 20日年化波动 | 21.35% | 11.95% | 82.39% | 范围内 |
| 成交量/20日均量 | 0.96倍 | 0.53倍 | 1.76倍 | 范围内 |
| 20日相对强弱 | -0.74% | -12.50% | 15.92% | 范围内 |
| 沪深300距120日均线 | -3.87% | -20.33% | 17.67% | 范围内 |
...
| 20日 | 39 | 未通过历史验证 | 预测未同时击败零收益MAE基线并达到高于50%的方向准确率，收益预测已停用 |
...
|---|---:|---|---:|---:|---:|
| 5日 | 40 | 样本达标 | 0.03% | -5.49% ～ 6.55% | 47.50% |
| 10日 | 40 | 样本达标 | -0.39% | -5.76% ～ 8.40% | 52.50% |
| 20日 | 40 | 样本达标 | -3.12% | -8.46% ～ 12.38% | 57.50% |

| 持有期 | 历史期间跌破起点概率 | 历史期间跌超5%概率 | 跌破后的低点窗口 |
|---|---:|---:|---:|
| 5日 | 57.50% | 15.00% | 第3～5日（n=23） |
| 10日 | 77.50% | 27.50% | 第4～10日（n=31） |
| 20日 | 85.00% | 52.50% | 第10～19日（n=34） |
...
验证状态：**极端动作样本不足**。规则在**154**个近似独立样本外日期上可计算；每次只使用该日以前已经实现的相似结果，相邻记录至少间隔10个交易日。
...
- 已实现的收缩后研究分数样本：**2455**。
- 全部重叠日标签的描述性方向准确率：**50.63%**。
- 每隔10个交易日抽取一次的近似非重叠样本：**246**；方向准确率：**51.22%**；Wilson 95%区间：**45.00% ～ 57.40%**。
- 收缩后概率型分数Brier：**0.2496**；原始模型：**0.2523**；固定50%概率基线：**0.2500**。Brier越低越好。
- Brier技能分数：**0.15%**（高于0才表示击败50%基线）；分段稳定性通过：**否**。
- ROC AUC：**0.5079**；0.5约等于随机排序。
...
| 最新尾窗 | 2026-07-01 | 2026-09-08 | 50 | 0.2560 | 0.8514 | 失败 |
...
|---|---:|---:|---:|---:|---:|---|
| 5日 | 339 | 3.43% | 3.31% | 48.08% | 70.80% | 未击败零收益基线 |
| 10日 | 154 | 4.91% | 4.82% | 50.65% | 66.23% | 未击败零收益基线 |
...
|---|---:|---:|---:|---:|---:|
| 质量门控定投 | ¥17,000.00 | ¥16,552.24 | ¥-447.76 | -2.63% | -14.55% |
| 固定定投 | ¥17,000.00 | ¥16,552.24 | ¥-447.76 | -2.63% | -14.55% |
| 质量门控影子策略（不可执行） | ¥17,000.00 | ¥16,582.50 | ¥-417.50 | -2.46% | -2.82% |

质量门控定投相对固定定投的市值差：**¥0.00**

影子Alpha（影子策略相对固定定投的市值差）：**¥30.26**。样本仅**47**个交易日，门控通过15日、实际持仓15日，当前只用于检验规则是否可能创造避险或超额收益。

![累计市值对比](performance.png)

## 数据口径与来源

- 模型特征与收益验证：`cnindex_official_980022`，覆盖2015-01-05至2026-09-22，共2850条。
