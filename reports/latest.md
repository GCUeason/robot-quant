...
- 逻辑回归原始分数：**56.76%**
- 向50%收缩25%后，未来10个交易日跑赢沪深300的预测概率型分数：**51.69%**
- 模型原始研究仓位：**50%**（不可执行）
...
- 逐日成熟样本门控：**未通过**；最近成熟样本50条，Brier=0.2523，AUC=0.5747；最近成熟窗口未同时通过Brier<0.25与AUC>0.5
- 不可执行影子目标仓位：**0%**。

## 基线优先门控

- 候选模型：**回顾性挑战者** `trend_risk_logistic_v3`；它是在查看既有历史后提出的候选，只能用未来新增数据继续验证。
- 模型研究概率：**51.69%**；批准概率固定为**50.00%**，表示当前没有可执行优势。
...
|---|---:|---:|---:|---|
| 5日收益 | -1.44% | -12.41% | 11.89% | 范围内 |
| 20日收益 | -5.58% | -24.38% | 25.37% | 范围内 |
| 60日收益 | -13.51% | -29.34% | 41.42% | 范围内 |
| 距20日均线 | -2.14% | -15.05% | 12.90% | 范围内 |
| 距60日均线 | -7.46% | -20.62% | 21.56% | 范围内 |
| 20日年化波动 | 42.75% | 11.95% | 82.39% | 范围内 |
| 成交量/20日均量 | 0.88倍 | 0.53倍 | 1.76倍 | 范围内 |
| 20日相对强弱 | -2.87% | -12.50% | 15.92% | 范围内 |
| 沪深300距120日均线 | -3.25% | -20.33% | 17.67% | 范围内 |
...
| 20日 | 40 | 未通过历史验证 | 预测未同时击败零收益MAE基线并达到高于50%的方向准确率，收益预测已停用 |
...
- 已实现的收缩后研究分数样本：**2444**。
- 全部重叠日标签的描述性方向准确率：**50.82%**。
- 每隔10个交易日抽取一次的近似非重叠样本：**245**；方向准确率：**51.43%**；Wilson 95%区间：**45.20% ～ 57.62%**。
- 收缩后概率型分数Brier：**0.2496**；原始模型：**0.2520**；固定50%概率基线：**0.2500**。Brier越低越好。
- Brier技能分数：**0.17%**（高于0才表示击败50%基线）；分段稳定性通过：**否**。
- ROC AUC：**0.5104**；0.5约等于随机排序。
...
| 固定分段 | 2026-06-24 | 2026-08-24 | 44 | 0.2527 | 0.6795 | 失败 |
...
|---|---:|---:|---:|---:|---:|---|
| 5日 | 336 | 3.44% | 3.31% | 47.92% | 70.54% | 未击败零收益基线 |
...
|---|---:|---:|---:|---:|---:|
| 质量门控定投 | ¥17,000.00 | ¥16,728.64 | ¥-271.36 | -1.60% | -12.23% |
| 固定定投 | ¥17,000.00 | ¥16,728.64 | ¥-271.36 | -1.60% | -12.23% |
| 质量门控影子策略（不可执行） | ¥17,000.00 | ¥16,582.50 | ¥-417.50 | -2.46% | -2.82% |

质量门控定投相对固定定投的市值差：**¥0.00**

影子Alpha（影子策略相对固定定投的市值差）：**¥-146.14**。样本仅**36**个交易日，门控通过15日、实际持仓15日，当前只用于检验规则是否可能创造避险或超额收益。

![累计市值对比](performance.png)

## 数据口径与来源

- 模型特征与收益验证：`cnindex_official_980022`，覆盖2015-01-05至2026-09-07，共2839条。
