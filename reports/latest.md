...
- 逻辑回归原始分数：**59.18%**
- 向50%收缩25%后，未来10个交易日跑赢沪深300的预测概率型分数：**52.29%**
- 模型原始研究仓位：**50%**（不可执行）
...
- 逐日成熟样本门控：**通过**；最近成熟样本50条，Brier=0.2465，AUC=0.5862；最近成熟窗口通过，仅允许不可执行影子卫星仓
- 不可执行影子目标仓位：**20%**。

## 基线优先门控

- 候选模型：**回顾性挑战者** `trend_risk_logistic_v3`；它是在查看既有历史后提出的候选，只能用未来新增数据继续验证。
- 模型研究概率：**52.29%**；批准概率固定为**50.00%**，表示当前没有可执行优势。
...
|---|---:|---:|---:|---|
| 5日收益 | -5.29% | -12.43% | 11.77% | 范围内 |
| 20日收益 | 1.90% | -24.36% | 25.38% | 范围内 |
| 60日收益 | -19.93% | -29.40% | 41.48% | 范围内 |
| 距20日均线 | -2.79% | -14.68% | 12.91% | 范围内 |
| 距60日均线 | -10.77% | -20.75% | 21.68% | 范围内 |
| 20日年化波动 | 51.43% | 11.93% | 82.45% | 范围内 |
| 成交量/20日均量 | 0.75倍 | 0.53倍 | 1.76倍 | 范围内 |
| 20日相对强弱 | 4.76% | -12.13% | 15.94% | 范围内 |
| 沪深300距120日均线 | -3.05% | -20.51% | 17.68% | 范围内 |
...
| 最新尾窗 | 2026-05-28 | 2026-08-06 | 50 | 0.2474 | 0.5881 | 通过 |
...
|---|---:|---:|---:|---:|---:|
| 质量门控定投 | ¥16,000.00 | ¥15,831.41 | ¥-168.59 | -1.05% | -10.26% |
| 固定定投 | ¥16,000.00 | ¥15,831.41 | ¥-168.59 | -1.05% | -10.26% |
| 质量门控影子策略（不可执行） | ¥16,000.00 | ¥15,648.43 | ¥-351.57 | -2.20% | -2.40% |

质量门控定投相对固定定投的市值差：**¥0.00**

影子Alpha（影子策略相对固定定投的市值差）：**¥-182.98**。样本仅**25**个交易日，门控通过11日、实际持仓10日，当前只用于检验规则是否可能创造避险或超额收益。

![累计市值对比](performance.png)

## 数据口径与来源

- 模型特征与收益验证：`cnindex_official_980022`，覆盖2015-01-05至2026-08-20，共2827条。
