...
- 逻辑回归原始分数：**62.07%**
- 向50%收缩25%后，未来10个交易日跑赢沪深300的预测概率型分数：**53.02%**
- 模型原始研究仓位：**50%**（不可执行）
...
- 逐日成熟样本门控：**未通过**；最近成熟样本50条，Brier=0.2523，AUC=0.4497；最近成熟窗口未同时通过Brier<0.25与AUC>0.5
- 不可执行影子目标仓位：**0%**。

## 基线优先门控

- 候选模型：**回顾性挑战者** `trend_risk_logistic_v3`；它是在查看既有历史后提出的候选，只能用未来新增数据继续验证。
- 模型研究概率：**53.02%**；批准概率固定为**50.00%**，表示当前没有可执行优势。
...
|---|---:|---:|---:|---|
| 5日收益 | 7.81% | -12.43% | 11.77% | 范围内 |
| 20日收益 | -9.24% | -24.36% | 25.38% | 范围内 |
| 60日收益 | -10.08% | -29.40% | 41.48% | 范围内 |
| 距20日均线 | 4.95% | -14.68% | 12.91% | 范围内 |
| 距60日均线 | -7.26% | -20.75% | 21.68% | 范围内 |
| 20日年化波动 | 53.62% | 11.93% | 82.45% | 范围内 |
| 成交量/20日均量 | 0.94倍 | 0.53倍 | 1.76倍 | 范围内 |
| 20日相对强弱 | -7.43% | -12.13% | 15.94% | 范围内 |
| 沪深300距120日均线 | -0.96% | -20.51% | 17.68% | 范围内 |
...
| 20日 | 28 | 未通过历史验证 | 预测未同时击败零收益MAE基线并达到高于50%的方向准确率，收益预测已停用 |
...
|---|---:|---|---:|---:|---:|
| 5日 | 40 | 样本达标 | 1.12% | -4.72% ～ 6.26% | 40.00% |
| 10日 | 40 | 样本达标 | 1.21% | -8.02% ～ 9.64% | 45.00% |
| 20日 | 40 | 样本达标 | -1.43% | -8.41% ～ 13.03% | 55.00% |
...
- 收缩后概率型分数Brier：**0.2495**；原始模型：**0.2519**；固定50%概率基线：**0.2500**。Brier越低越好。
- Brier技能分数：**0.18%**（高于0才表示击败50%基线）；分段稳定性通过：**否**。
- ROC AUC：**0.5111**；0.5约等于随机排序。
...
| 最新尾窗 | 2026-05-15 | 2026-07-24 | 50 | 0.2519 | 0.4675 | 失败 |
...
影子Alpha（影子策略相对固定定投的市值差）：**¥-831.51**。样本仅**16**个交易日，门控通过3日、实际持仓3日，当前只用于检验规则是否可能创造避险或超额收益。

![累计市值对比](performance.png)

## 数据口径与来源

- 模型特征与收益验证：`cnindex_official_980022`，覆盖2015-01-05至2026-08-07，共2818条。
